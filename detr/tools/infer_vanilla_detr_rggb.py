...
# -*- coding: utf-8 -*-
"""
Vanilla DETR (RGGB) standalone inference.
...
from tools.train_vanilla_detr_rggb import VanillaDETRRGGB
...
def _extract_state_dict(blob: Any) -> Dict[str, torch.Tensor]:
    ...
    candidates: List[Any] = []
    for key in ("model", "state_dict", "detector", "detr"):
    ...
    for cand in candidates:
        if not isinstance(cand, dict):
            continue
        num_tensors = sum(1 for v in cand.values() if torch.is_tensor(v))
        if num_tensors >= 8:
            picked = cand
            break
...
def _build_model_from_ckpt_args(blob: Dict[str, Any], cli: argparse.Namespace) -> Tuple[VanillaDETRRGGB, Dict[str, Any], int]:
    ...
    num_queries = _get_arch_value(ckpt_args, "num_queries", cli.num_queries, 100, int)
    d_model = _get_arch_value(ckpt_args, "d_model", cli.d_model, 256, int)
    nhead = _get_arch_value(ckpt_args, "nhead", cli.nhead, 8, int)
    enc_layers = _get_arch_value(ckpt_args, "enc_layers", cli.enc_layers, 6, int)
    dec_layers = _get_arch_value(ckpt_args, "dec_layers", cli.dec_layers, 6, int)
    dim_feedforward = _get_arch_value(ckpt_args, "dim_feedforward", cli.dim_feedforward, 2048, int)
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser("Standalone vanilla DETR (RGGB) inference")
